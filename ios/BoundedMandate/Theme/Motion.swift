...
enum Motion {
    ...
    static func enter(_ duration: TimeInterval = 0.26) -> Animation {
    ...
    static let morph = Animation.timingCurve(0.32, 0.72, 0, 1, duration: 0.42)

    /// Coming back is a system response, not a decision, so it snaps.
    /// Asymmetry is the point: deliberate in, quick out.
    static let unmorph = Animation.timingCurve(0.32, 0.72, 0, 1, duration: 0.26)
    ...
    static let follow = Animation.spring(duration: 0.28, bounce: 0.1)
    ...
    static func respectful(_ animation: Animation, reduced: Bool) -> Animation {
        reduced ? .easeOut(duration: 0.18) : animation
    }
}
...
private struct Arrival: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            .opacity(progress)
            .scaleEffect(reduced ? 1 : 0.98 + 0.02 * progress, anchor: .bottom)
            .offset(y: reduced ? 0 : 8 * (1 - progress))
    }
}
